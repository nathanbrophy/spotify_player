...
class Mopidy extends EventEmitter {
  ...
  _configure(settings) {
    ...
    const protocol = typeof document !== "undefined" && document.location.protocol === "https:" ? "wss://" : "ws://";
    const currentHost = typeof document !== "undefined" && document.location.host || "localhost";
    newSettings.webSocketUrl = settings.webSocketUrl || `${protocol}${currentHost}/mopidy/ws`;
  ...
  off(...args) {
    if (args.length === 0) {
      ...
      if (typeof arg === "string") {
        ...
        throw Error("Expected no arguments, a string, or a string and a listener.");
      }
    } else {
  ...
  connect() {
    ...
    this._webSocket = this._settings.webSocket || new Mopidy.WebSocket(this._settings.webSocketUrl);
  ...
  _send(message) {
    switch (this._webSocket.readyState) {
      case Mopidy.WebSocket.CONNECTING:
        return Promise.reject(new Mopidy.ConnectionError("WebSocket is still connecting"));
      case Mopidy.WebSocket.CLOSING:
        return Promise.reject(new Mopidy.ConnectionError("WebSocket is closing"));
  ...
  _handleMessage(message) {
    try {
      ...
      if (Object.hasOwnProperty.call(data, "id")) {
        ...
        this._console.warn(`Unknown message type received. Message was: ${message.data}`);
      }
    } catch (error) {
      if (error instanceof SyntaxError) {
        this._console.warn(`WebSocket message parsing failed. Message was: ${message.data}`);
      } else {
        throw error;
      }
    }
  ...
  _handleResponse(responseMessage) {
    if (!Object.hasOwnProperty.call(this._pendingRequests, responseMessage.id)) {
      this._console.warn("Unexpected response received. Message was:", responseMessage);
      return;
    }
    ...
    if (Object.hasOwnProperty.call(responseMessage, "result")) {
      ...
      reject(error);
      this._console.warn("Response without 'result' or 'error' received. Message was:", responseMessage);
    }
  }
  ...
  _getApiSpec() {
    return this._send({ method: "core.describe" }).then(this._createApi.bind(this)).catch(this._handleWebSocketError);
  }
  ...
  _createApi(methods) {
    const caller = method => (...args) => {
      ...
      if (args.length > 1) {
        return Promise.reject(new Error("Expected zero arguments, a single array, or a single object."));
      }
