            ...
            mopidy.playback.getCurrentTrack().then(track => {
              ...
              if (this.state.currentTrack) {
                ...
                mopidy.playlists.getPlaylists().then(function (playlists) {
                  ...
                  if (playlist != null) {
                    var currentI = playlist.tracks.length,
                        tempVal,
                        randomI;
                    while (0 != currentI) {
...
var prevTrackImages = [];
var nextTrackImages = [];
const MAX_NEXT_IMAGES = 2;
const MAX_PREV_IMAGES = 2;
function updateCoverArt(trackUri, imgs) {

  window.music.mopidy.tracklist.getTracks().then(tracks => {
    for (var i = 0; i < MAX_NEXT_IMAGES; i++) {
      console.log(tracks[i]);
      window.music.mopidy.library.getImages([tracks[i].uri]).then(result => {
        console.log(tracks[i].uri);
        const [nextImage] = result[tracks[i].uri];
      });
    }
  });
